...
<div class="page-action-area">
    ...
    <div class="header-actions">
        <button class="btn-light job-list-toolbar__create" type="button" onclick="copyJoiningFormLink()" style="display: inline-flex; align-items: center; gap: 8px; font-weight: 600;">
            <i data-lucide="copy"></i>
            <span>Copy Joining Form Link</span>
        </button>
    </div>
</div>

<script>
function copyJoiningFormLink() {
    const url = '<?= \App\Core\View::url('joining-form') ?>';
    const showSuccess = () => {
        if (typeof Swal !== 'undefined') {
            Swal.fire({
                icon: 'success',
                title: 'Link Copied!',
                text: 'Public Joining Form link copied to clipboard.',
                timer: 1800,
                showConfirmButton: false
            });
        } else {
            alert('Public Joining Form link copied to clipboard.');
        }
    };
    ...
    function fallbackCopy() {
        ...
        if (copyOk) {
            ...
            prompt('Copy Joining Form URL:', url);
        }
    }
}
</script>
...
<script src="<?= \App\Core\View::asset('js/admin/new-joining.js') ?>"></script>
