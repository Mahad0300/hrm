...
<div class="page-action-area">
    <button type="button" class="btn-primary" id="openLeaveSettingsBtn" onclick="openModal('applyLeaveModal')">
                ...
                <tbody id="leaveTableBody">
                    <?php if (count($all_requests) > 0): ?>
                        <?php foreach ($all_requests as $lr): 
                            $emp_id_display = sprintf('EMP-%03d', (int) $lr['employee_id']);
                            $middle_name_part = !empty($lr['middle_name']) ? $lr['middle_name'] . ' ' : '';
                            $leave_user_name = trim($lr['first_name'] . ' ' . $middle_name_part . $lr['last_name']);
                            $leave_user_avatar = !empty($lr['profile_pic']) && file_exists(STORAGE_DIR . '/' . $lr['profile_pic']) ? \App\Core\View::upload($lr['profile_pic']) : \App\Core\View::image('profile-image/default-avatar.svg');
                            
                            $start_ts = strtotime($lr['start_date']);
                            $end_ts = strtotime($lr['end_date']);
                            $days = \App\Helpers\LeaveAttendanceSync::countWorkingLeaveDays($lr['start_date'], $lr['end_date']);
                            ...
                            <td class="text-right px-30">
                                <div class="btn-group justify-end">
                                    <button class="action-btn action-btn-view" title="View Details"
                                        onclick="openLeaveDetailModal(<?= $lr['id'] ?>, '<?= htmlspecialchars($leave_user_name, ENT_QUOTES, 'UTF-8') ?>','<?= htmlspecialchars($lr['leave_type_name'], ENT_QUOTES, 'UTF-8') ?>',<?= $days ?>,'<?= date("d M Y", $start_ts) ?>','<?= date("d M Y", $end_ts) ?>','<?= $safe_admin_note_js ?>','<?= $doc_path_js ?>','<?= $is_img ? 'image' : 'pdf' ?>', '<?= $lr['status'] ?>', '<?= $safe_reason_js ?>')"><i data-lucide="eye" size="14"></i></button>
                    ...
                    ?>
                        <tr data-id="<?= $lr['id'] ?>" data-status="Pending">
                            ...
                            <td class="text-right px-30">
                                <div class="btn-group justify-end">
                                    <button class="action-btn action-btn-view" title="View Details"
                                        onclick="openLeaveDetailModal(<?= $lr['id'] ?>, '<?= htmlspecialchars($leave_user_name, ENT_QUOTES, 'UTF-8') ?>','<?= htmlspecialchars($lr['leave_type_name'], ENT_QUOTES, 'UTF-8') ?>',<?= $days ?>,'<?= date("d M Y", $start_ts) ?>','<?= date("d M Y", $end_ts) ?>','<?= $safe_admin_note_js ?>','<?= $doc_path_js ?>','<?= $is_img ? 'image' : 'pdf' ?>', 'Pending', '<?= $safe_reason_js ?>')"><i data-lucide="eye" size="14"></i></button>
    ...
    <div class="modal-content premium leave-detail-modal">
        ...
        <div class="modal-body">
            ...
            <div class="leave-detail-section">
                ...
                <p class="font-14 text-dark m-0 leave-detail-reason" id="leaveDetailReason" style="word-break: break-word; white-space: pre-wrap;">—</p>
            </div>
...
<script src="<?= \App\Core\View::asset('js/shared/leave-management.js') ?>"></script>
