...
$leave_user_id = sprintf('EMP-%03d', (int) $emp_id);
$leave_user_avatar = !empty($emp_data['profile_pic']) && file_exists(STORAGE_DIR . '/' . $emp_data['profile_pic']) ? \App\Core\View::upload($emp_data['profile_pic']) : \App\Core\View::image('profile-image/default-avatar.svg');
...
$leave_types = $pdo->query('SELECT id, name, days_per_year FROM leave_types ORDER BY id ASC')->fetchAll(PDO::FETCH_ASSOC);
$leave_types_available = !empty($leave_types);
?>
    ...
    <div class="modal-content premium wide-md">
        ...
        <form id="applyLeaveForm">
            <div class="modal-body p-30">
                <div class="form-grid-2 gap-20">
                    <div class="form-group col-span-2">
                        ...
                        <select class="form-control bg-white-input" id="applyLeaveType" name="leave_type" required <?= $leave_types_available ? '' : 'disabled' ?>>
                            <option value="">Select leave type</option>
                            <?php foreach ($leave_types as $lt): ?>
                            <option value="<?= htmlspecialchars($lt['name'], ENT_QUOTES, 'UTF-8') ?>">
                                <?= htmlspecialchars($lt['name'], ENT_QUOTES, 'UTF-8') ?>
                                (<?= (int) $lt['days_per_year'] ?> days/year)
                            </option>
                            <?php endforeach; ?>
                        </select>
                        <?php if (!$leave_types_available): ?>
                        <p class="font-12 text-danger m-0 mt-8">Leave settings are not configured yet. Please contact HR or Admin.</p>
                        <?php endif; ?>
    </div>
            ...
            <div class="modal-footer flex-between flex-wrap gap-12">
                ...
                <button type="submit" class="btn-primary" <?= $leave_types_available ? '' : 'disabled' ?>>
            ...
            <div class="modal-body p-30">
                ...
                <div class="form-grid-2 gap-20">
                    <div class="form-group col-span-2">
                        ...
                        <select class="form-control bg-white-input" id="editLeaveType" name="leave_type" required <?= $leave_types_available ? '' : 'disabled' ?>>
                            <option value="">Select leave type</option>
                            <?php foreach ($leave_types as $lt): ?>
                            <option value="<?= htmlspecialchars($lt['name'], ENT_QUOTES, 'UTF-8') ?>">
                                <?= htmlspecialchars($lt['name'], ENT_QUOTES, 'UTF-8') ?>
...
<script>
    window.LEAVE_TYPES_AVAILABLE = <?= $leave_types_available ? 'true' : 'false' ?>;
</script>
<script src="<?= \App\Core\View::asset('js/user/leave-management.js') ?>"></script>
