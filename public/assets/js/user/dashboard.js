...
function fetchRecentNotifications() {
    ...
    fetch('/assets/api/stats_handler.php?action=get_recent_notifications')
        .then(res => res.json())
        .then(res => {
            if (res.status === 'success') {
                ...
                container.innerHTML = data.map(not => `
                    <a href="${window.HRM.url('notifications')}" class="udash-feed__item">
                        <span class="udash-feed__body">
                            <span class="udash-feed__title"><span class="udash-feed__emoji" aria-hidden="true">🔔</span>${escapeHtml(not.title)}</span>
                            <span class="udash-feed__meta">
                                ${escapeHtml(not.message.substring(0, 45))}${not.message.length > 45 ? '...' : ''} · ${escapeHtml(not.time_ago)}
                            </span>
                        </span>
                    </a>
                `).join('');
...
function fetchLatestAnnouncements() {
    ...
    fetch('/assets/api/stats_handler.php?action=get_latest_announcements')
        .then(res => res.json())
        .then(res => {
            if (res.status === 'success') {
                const data = res.data;
                ...
                container.innerHTML = data.map(ann => `
                    <a href="${window.HRM.url('announcements')}" class="udash-feed__item">
                        <span class="udash-feed__body">
                            <span class="udash-feed__title"><span class="udash-feed__emoji" aria-hidden="true">${getEmoji(ann.type)}</span>${escapeHtml(ann.title)}</span>
                            <span class="udash-feed__meta">
                                <span class="udash-feed__badge udash-feed__badge--${getBadgeClass(ann.type)}">${escapeHtml(ann.type)}</span> 
                                ${escapeHtml(ann.author_name)} · ${escapeHtml(ann.time_ago)}
                            </span>
                        </span>
                    </a>
                `).join('');
...
function fetchPersonalStats() {
    fetch('/assets/api/stats_handler.php?action=get_personal_overview')
        .then(res => res.json())
        .then(res => {
            if (res.status === 'success') {
                const d = res.data;
                ...
                if (att) {
                    ...
                    if (badge && badge.parentElement) badge.parentElement.style.display = 'none';
                    if (liveTimerInterval) clearInterval(liveTimerInterval);
                }
            }
        })
    ...
    fetch('/assets/api/stats_handler.php?action=get_announcements_notifications')
        .then(res => res.json())
        .then(res => {
            if (res.status === 'success') {
                const d = res.data;
                ...
                if (annContainer) {
                    if (d.announcements.length === 0) {
                        ...
                        annContainer.innerHTML = d.announcements.map(ann => `
                            <a href="${window.HRM.url('announcements')}" class="udash-feed__item">
                                <span class="udash-feed__body">
                                    <span class="udash-feed__title"><span class="udash-feed__emoji" aria-hidden="true">📢</span>${escapeHtml(ann.title)}</span>
                                    <span class="udash-feed__meta">
                                        <span class="udash-feed__badge ${ann.category === 'Holiday' ? 'udash-feed__badge--holiday' : 'udash-feed__badge--update'}">
                                            ${escapeHtml((ann.category || '').toUpperCase())}
                                        </span> 
                                        · ${escapeHtml(formatFullDate(ann.event_date))}
                                    </span>
                                </span>
                            </a>
                        `).join('');
                ...
                if (notContainer) {
                    if (d.notifications.length === 0) {
                        ...
                        notContainer.innerHTML = d.notifications.map(n => `
                            <a href="${window.HRM.url('notifications')}" class="udash-feed__item">
                                <span class="udash-feed__body">
                                    <span class="udash-feed__title"><span class="udash-feed__emoji" aria-hidden="true">🔔</span>${escapeHtml(n.title)}</span>
                                    <span class="udash-feed__meta">${escapeHtml(n.message.substring(0, 50))}${n.message.length > 50 ? '...' : ''} · ${escapeHtml(formatFullDate(n.created_at))}</span>
                                </span>
                            </a>
                        `).join('');
                    }
...
// Expose globally for real-time WebSocket updates
window.fetchPersonalStats = fetchPersonalStats;
window.refreshAttendanceStatus = fetchPersonalStats;
