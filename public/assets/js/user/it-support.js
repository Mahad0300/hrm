...
const ITSupport = (() => {
    const apiUrl = window.IT_SUPPORT_API || '/assets/api/it-support-handler.php';
    ...
    const init = () => {
        ...
        fetchTickets();
        showPlaceholder();
        // Live updates handled via WebSocket (support_updated event)
    };
        ...
        if (ticket.messages.length !== lastMessageCount) {
            ...
            ticket.messages.forEach(m => {
                ...
                if (m.is_system || m.text.startsWith('Ticket Re-opened.')) {
                    ...
                    const senderNameHtml = !isRight ? `<div style="font-size: 10px; color: #5b5b5b; margin-bottom: 2px; font-weight: 600;">${m.sender}</div>` : '';
...
$(document).ready(() => {
    ITSupport.init();
    window.refreshITSupport = ITSupport.refresh;
});
