...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function fetchLog() {
        ...
        fetch(window.HRM.api(`attendance_handler.php?action=fetch_log&emp_id=${empId}&month=${month}`))
            .then(res => res.json())
            .then(res => {
    ...
    function updateStats() {
        ...
        allLogs.forEach(log => {
            ...
            if (status === 'ON TIME') countOnTime++;
            else if (status === 'LATE IN') countLateIn++;
            else if (status === 'ABSENT') countAbsent++;
            else if (status === 'HALF DAY') countHalfDay++;
            else if (status === 'LEAVE') countLeave++;
            else if (status === 'HOLIDAY') countHoliday++;
        });

        const onTimeEl = document.getElementById('stat-count-ontime');
        const lateInEl = document.getElementById('stat-count-latein');
        const absentEl = document.getElementById('stat-count-absent');
        const halfDayEl = document.getElementById('stat-count-halfday');
        const leaveEl = document.getElementById('stat-count-leave');
        const holidayEl = document.getElementById('stat-count-holiday');

        if (onTimeEl) onTimeEl.textContent = countOnTime;
        if (lateInEl) lateInEl.textContent = countLateIn;
        if (absentEl) absentEl.textContent = countAbsent;
        if (halfDayEl) halfDayEl.textContent = countHalfDay;
        if (leaveEl) leaveEl.textContent = countLeave;
        if (holidayEl) holidayEl.textContent = countHoliday;
    }
    ...
    function renderTable() {
        ...
        paginated.forEach(r => {
            ...
            row.innerHTML = `
                ...
                <td>${r.shift_name ? `${r.shift_name} (${formatSimpleTime(r.shift_start)} - ${formatSimpleTime(r.shift_end)})` : '-'}</td>
                <td>${inTime}</td>
                <td>${outTime}</td>
    ...
    window.openEditModal = function(data) {
        ...
        const inTime = data.clock_in ? formatTimeOnly(data.clock_in) : '--:--';
        const outTime = data.clock_out ? formatTimeOnly(data.clock_out) : '--:--';
    ...
    window.closeAttendanceModal = function() {
        ...
        if (typeof window.closeAllTimePickers === 'function') {
            window.closeAllTimePickers();
        }
    };
    ...
    document.getElementById('saveAttendanceBtn').onclick = function() {
        ...
        fetch(window.HRM.api('attendance_handler.php?action=update_attendance'), {
            method: 'POST',
        ...
        .then(res => {
            ...
            if (res.status === 'success') {
                ...
                if (typeof window.closeAttendanceModal === 'function') {
    ...
    fetchLog();
    // Live updates handled via WebSocket (attendance_updated event)
});
