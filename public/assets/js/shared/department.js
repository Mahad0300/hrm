document.addEventListener('DOMContentLoaded', () => {
    ...
    window.editDept = (id) => {
        ...
        if (dept) {
            ...
            // Dynamically filter department heads to only show employees belonging to this department
            const editDeptHeadSelect = document.getElementById('editDeptHead');
            if (editDeptHeadSelect) {
                const filteredHeads = allHeads.filter(h => String(h.department_id) === String(id));
                editDeptHeadSelect.innerHTML = '<option value="">Select Head</option>' + 
                    filteredHeads.map(h => {
                        const name = [h.first_name, h.middle_name, h.last_name].filter(v => v && v.trim() !== '').join(' ');
                        return `<option value="${h.id}">${name}</option>`;
                    }).join('');
                editDeptHeadSelect.value = dept.head || '';
            }
