...
function renderActivityTable(logs) {
    ...
    tableBody.innerHTML = logs.map(log => {
        ...
        // Format ID as EMP-005 (matching 3-digit format across the HRM system)
        const formattedId = `EMP-${String(log.emp_code).padStart(3, '0')}`;
        const employeeName = [log.first_name, log.middle_name, log.last_name].filter(v => v && String(v).trim() !== '').join(' ');

        const escape = window.HRM_UTILS && typeof window.HRM_UTILS.escapeHtml === 'function' 
            ? window.HRM_UTILS.escapeHtml 
            : (str) => String(str || '').replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;');

        const safeEmpName = escape(employeeName);
        const safeModule = escape(log.module);
        const safeAction = escape(log.action);
        const safeDetails = escape(log.details);
        const safeIp = escape(log.ip_address === '::1' ? 'Localhost' : log.ip_address);
            ...
            <tr>
                <td>
                    <div class="emp-profile">
                        ...
                        <div class="emp-info">
                            <span class="name">${safeEmpName}</span>
                ...
                <td>
                    <span class="badge badge-light">${safeModule}</span>
                </td>
                <td>
                    <span class="badge ${getActionBadgeClass(log.action)}">${safeAction}</span>
                </td>
                <td class="allow-wrap font-13">${safeDetails}</td>
                <td><code class="font-12">${safeIp}</code></td>
            </tr>
        `;
    }).join('');
}
