...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function toggleDeptPill(pill, container) {
        ...
        if (pill.dataset.dept === 'everyone') {
            ...
            hiddenInput.value = activePills.length > 0 ? activePills.join(',') : '';
        }
    }
    ...
    function renderAnnouncements(data) {
        ...
        data.forEach(ann => {
            ...
            let authorImg = '../images/profile-image/default-avatar.svg';
            if (ann.profile_pic) {
                authorImg = ann.profile_pic.startsWith('http') || ann.profile_pic.startsWith('/') 
                    ? ann.profile_pic 
                    : (ann.profile_pic.startsWith('../') ? ann.profile_pic : '../' + ann.profile_pic);
            }
            ...
            card.innerHTML = `
                ...
                <div class="announcement-content">
                    ...
                    <h3 class="mb-12 font-18 font-700">${ann.title}</h3>
                    <p class="font-14 text-light mb-20" style="line-height: 1.6;">${stripHtml(ann.content).substring(0, 140)}${stripHtml(ann.content).length > 140 ? '...' : ''}</p>
                </div>
                <div class="announcement-footer">
                    <div class="flex-center gap-10">
                        <img src="${authorImg}" 
                             class="icon-box-sm" style="border-radius: 50%; object-fit: cover;" onerror="this.src='../images/profile-image/default-avatar.svg'">
    ...
    window.openEditAnnouncementModal = function(id) {
        ...
        // Activate Dept pills
        const targetDepts = (ann.target_depts || '').trim();
        const isEveryone = targetDepts === '' || ['everyone', 'all'].includes(targetDepts.toLowerCase());
        const depts = isEveryone ? [] : targetDepts.split(',').map(d => d.trim()).filter(Boolean);
        const container = document.getElementById('editDeptSelection');
        container.querySelectorAll('.category-pill').forEach(p => {
            if (p.dataset.dept === 'everyone') {
                p.classList.toggle('active', isEveryone);
            } else {
                p.classList.toggle('active', !isEveryone && depts.includes(p.dataset.dept));
            }
        });
    ...
    window.viewAnnouncementDetail = function(id, source) {
        ...
        let viewAuthorImg = '../images/profile-image/default-avatar.svg';
        if (item.profile_pic) {
            viewAuthorImg = item.profile_pic.startsWith('http') || item.profile_pic.startsWith('/')
                ? item.profile_pic
                : (item.profile_pic.startsWith('../') ? item.profile_pic : '../' + item.profile_pic);
        }
        document.getElementById('view_ann_author_img').src = viewAuthorImg;

        // Target depts in view
        const deptContainer = document.getElementById('view_ann_depts');
        if (deptContainer) {
            deptContainer.innerHTML = '';
            const depts = (item.target_depts || 'Everyone').split(',').map(d => d.trim()).filter(Boolean);
            depts.forEach(d => {
                const pill = document.createElement('div');
    ...
    window.refreshAnnouncements = fetchAnnouncements;
});
