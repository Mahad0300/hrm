...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (yearDisplay && monthInputHidden) {
        ...
        const updateMonthInput = () => {
            ...
            if (document.getElementById('editPayrollModal')?.classList.contains('active')) {
                reloadPayrollEditDetails();
            }
        };
    ...
    window.refreshPayroll = fetchPayroll;
});
...
let editPayrollEmpId = null;
let editPayrollEmpName = '';

function formatPayrollCycleLabel(cycle) {
...
function populatePayrollEditForm(data, employee, cycle, empName, empId) {
    const subtitle = document.getElementById('editPayrollSubtitle');
    const form = document.getElementById('editPayrollForm');
    if (!form) return;

    const cycleLabel = formatPayrollCycleLabel(cycle);
    subtitle.textContent = cycleLabel
        ? `Updating payroll for ${empName} (EMP-${String(empId).padStart(3, '0')}) • ${cycleLabel}`
        : `Updating payroll for ${empName} (EMP-${String(empId).padStart(3, '0')})`;

    form.querySelector('#editEmpId').value = empId;
    form.querySelector('#editMonth').value = data.month_year || form.querySelector('#monthInput').value;

    form.querySelector('[name="leaves"]').value = Math.round(data.absents_count !== undefined ? data.absents_count : (data.leaves_count || 0));
    ...
    updateSalaryInfoPanel(data, employee);
    addLiveCalculationListeners(employee.salary);
}

async function reloadPayrollEditDetails() {
    if (!editPayrollEmpId) return;

    const month = document.getElementById('monthInput')?.value || document.getElementById('editMonth')?.value;
    if (!month) return;

    document.getElementById('editMonth').value = month;

    try {
        const response = await fetch(`/assets/api/payroll_handler.php?action=get_payroll_details&employee_id=${editPayrollEmpId}&month=${month}`);
        const result = await response.json();
        if (result.status === 'success') {
            populatePayrollEditForm(result.data, result.employee, result.cycle, editPayrollEmpName, editPayrollEmpId);
        }
    } catch (error) {
        console.error('Error reloading payroll details:', error);
    }
}
...
window.openEditPayrollModal = async function(empId, empName, month) {
    ...
    editPayrollEmpId = empId;
    editPayrollEmpName = empName;
    ...
    try {
        ...
        if (result.status === 'success') {
            ...
            document.querySelectorAll('.month-item').forEach(item => {
                item.classList.toggle('active', parseInt(item.getAttribute('data-month'), 10) === monthIdx);
            });

            populatePayrollEditForm(result.data, result.employee, result.cycle, empName, empId);
        } else {
...
window.viewPayslip = function(empId, month, empName) {
    const nameSlug = String(empName)
        .toLowerCase()
        .replace(/[^a-z0-9]+/gi, '-')
        .replace(/^-+|-+$/g, '');
    const baseUrl = (window.HRM && window.HRM.url) ? window.HRM.url('payslip/print') : '/payslip/print';
    const payslipUrl = baseUrl + '?name=' + encodeURIComponent(nameSlug) + '&month=' + encodeURIComponent(month);
