...
document.addEventListener('DOMContentLoaded', function () {
        ...
        fetch(`/assets/api/attendance_handler.php?action=fetch_daily&date=${date}`)
            ...
            .then(res => {
                if (res.status === 'success') {
                    ...
                    if (window.lastAttendanceDataStr !== newDataStr) {
                        allData = res.data;
                        applyFilters();
                        updateStats();
                        window.lastAttendanceDataStr = newDataStr;
                    }
                } else {
    ...
    function applyFilters() {
        ...
        filteredData = allData.filter(r => {
            ...
            if (statusTerm === 'PRESENT') {
                matchesStatus = (r.status === 'ON TIME' || r.status === 'LATE IN' || r.status === 'HALF DAY');
            } else if (statusTerm === 'ABSENT') {
                matchesStatus = (r.status === 'ABSENT' || r.status === 'NO RECORD');
            } else if (statusTerm) {
                matchesStatus = (r.status === statusTerm);
            }
    ...
    function updateStats() {
        const stats = {
            ...
            absent: allData.filter(r => r.status === 'ABSENT' || r.status === 'NO RECORD').length,
    ...
    function renderTable() {
        ...
        paginated.forEach(r => {
            ...
            const nameSlug = String(r.full_name || '')
                .trim()
                .toLowerCase()
                .replace(/[^a-z0-9]+/gi, '-')
                .replace(/^-+|-+$/g, '');
            const logUrl = typeof window.HRM !== 'undefined' && window.HRM.url 
                ? window.HRM.url('attendance/log?name=' + encodeURIComponent(nameSlug)) 
                : 'attendance/log?name=' + encodeURIComponent(nameSlug);
            ...
            row.innerHTML = `
                ...
                <td class="text-right px-30">
                    <div class="btn-group justify-end gap-8">
                        <a href="${logUrl}" class="action-btn action-btn-view" title="View Logs">
    ...
    // --- Stat Card Interactive Filters ---
    const cardStatusMap = [
        { el: document.getElementById('statPresent'), status: 'PRESENT', color: '#10b981' },
        { el: document.getElementById('statAbsent'), status: 'ABSENT', color: '#ef4444' },
        { el: document.getElementById('statHalfDay'), status: 'HALF DAY', color: '#3b82f6' },
        { el: document.getElementById('statLate'), status: 'LATE IN', color: '#f59e0b' }
    ];

    cardStatusMap.forEach(item => {
        if (!item.el) return;
        const card = item.el.closest('.stat-card');
        if (!card) return;

        card.style.cursor = 'pointer';
        card.style.transition = 'all 0.25s ease';
        card.title = `Click to filter by ${item.status}`;

        card.addEventListener('mouseenter', function () {
            if (statusFilter.value !== item.status) {
                card.style.transform = 'translateY(-3px)';
                card.style.boxShadow = `0 8px 20px -4px ${item.color}30, 0 4px 8px -2px ${item.color}15`;
            }
        });
        ...
        card.addEventListener('click', function () {
            if (statusFilter.value === item.status) {
                statusFilter.value = ''; // Toggle off if clicked again
            } else {
    ...
    function highlightActiveStatCard() {
        ...
        cardStatusMap.forEach(item => {
            ...
            if (val && val === item.status) {
                ...
                card.style.boxShadow = `0 12px 24px -4px ${item.color}45, 0 4px 8px -2px ${item.color}25`;
                card.style.outline = `2px solid ${item.color}`;
    ...
    function fetchBulkEmployees() {
        ...
        const isRange = document.getElementById('bulkDateRangeToggle') ? document.getElementById('bulkDateRangeToggle').checked : false;
        const date = isRange ? (document.getElementById('bulkStartDateInput').value || document.getElementById('bulkDateInput').value) : document.getElementById('bulkDateInput').value;
    ...
    // --- Flatpickr Calendar & Mode Switcher Setup ---
    let currentCalendarMode = 'single'; // 'single' or 'range'
    let selectedSingleDate = '';
    let selectedStartDate = '';
    let selectedEndDate = '';
    let bulkFlatpickr = null;

    const datePickerElem = document.getElementById('bulkDatePickerInput');
    const modeSingleBtn = document.getElementById('modeSingleBtn');
    const modeRangeBtn = document.getElementById('modeRangeBtn');

    function initFlatpickr(mode = 'single') {
        if (bulkFlatpickr) {
            bulkFlatpickr.destroy();
        }
        currentCalendarMode = mode;

        if (typeof flatpickr === 'undefined' || !datePickerElem) return;

        const defaultVal = datePickerElem.value || 'today';
        let defaultDates = defaultVal;

        if (mode === 'range') {
            defaultDates = (selectedStartDate && selectedEndDate) 
                ? [selectedStartDate, selectedEndDate] 
                : [defaultVal, defaultVal];
        } else {
            defaultDates = selectedSingleDate || defaultVal;
        }

        bulkFlatpickr = flatpickr(datePickerElem, {
            mode: mode,
            dateFormat: "Y-m-d",
            ...
            onChange: function(selectedDates, dateStr, instance) {
                if (mode === 'single') {
                    if (selectedDates.length > 0) {
                        selectedSingleDate = instance.formatDate(selectedDates[0], "Y-m-d");
                    }
                } else if (mode === 'range') {
        ...
        // Pre-fill initial selection state
        if (mode === 'single' && bulkFlatpickr.selectedDates.length > 0) {
            selectedSingleDate = bulkFlatpickr.formatDate(bulkFlatpickr.selectedDates[0], "Y-m-d");
        } else if (mode === 'range' && bulkFlatpickr.selectedDates.length === 2) {
            selectedStartDate = bulkFlatpickr.formatDate(bulkFlatpickr.selectedDates[0], "Y-m-d");
            selectedEndDate = bulkFlatpickr.formatDate(bulkFlatpickr.selectedDates[1], "Y-m-d");
        }
    }
    ...
    if (modeSingleBtn && modeRangeBtn) {
        modeSingleBtn.addEventListener('click', function(e) {
            e.preventDefault();
            modeSingleBtn.classList.add('active');
            modeRangeBtn.classList.remove('active');
            if (datePickerElem) datePickerElem.placeholder = 'Select date...';
            initFlatpickr('single');
            fetchBulkEmployees();
        });

        modeRangeBtn.addEventListener('click', function(e) {
            e.preventDefault();
            modeRangeBtn.classList.add('active');
            modeSingleBtn.classList.remove('active');
            if (datePickerElem) datePickerElem.placeholder = 'Select date range...';
            initFlatpickr('range');
            fetchBulkEmployees();
        });
    }
    ...
    function fetchBulkEmployees() {
        ...
        let date = selectedSingleDate;
        if (currentCalendarMode === 'range') {
            date = selectedStartDate || selectedSingleDate || (datePickerElem ? datePickerElem.value : '');
        } else {
            date = selectedSingleDate || (datePickerElem ? datePickerElem.value : '');
        }
    ...
    document.getElementById('saveBulkBtn').onclick = function(e) {
        ...
        const isRange = (currentCalendarMode === 'range');
        let startDate = selectedStartDate;
        let endDate = selectedEndDate;
        let date = selectedSingleDate || (datePickerElem ? datePickerElem.value : '');
        ...
        if (isRange) {
            if (!startDate || !endDate) {
                showToast('Please select both Start Date and End Date on the calendar.', 'error');
                return;
            }
            if (startDate > endDate) {
    ...
    };    // Expose for WebSocket real-time updates
    window.refreshAttendanceTable = fetchAttendance;
});
