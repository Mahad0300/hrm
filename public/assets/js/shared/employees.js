...
function getEmployeeProfileUrl(emp) {
    ...
    const path = slug
        ? `employees/profile?name=${encodeURIComponent(slug)}`
        : `employees/profile?id=${encodeURIComponent(emp.id)}`;
    return typeof window.HRM !== 'undefined' && window.HRM.url ? window.HRM.url(path) : '/' + path;
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderEmployees(employees) {
        ...
        tableBody.innerHTML = employees.map(emp => {
            ...
            const empNameEscaped = getEmployeeFullName(emp).replace(/'/g, "\\'");
            const actionsHtml = isExitOnly
                ...
                : `
                            <button class="action-btn action-btn-view" title="View Details" onclick="window.location.href='${profileUrl}'"><i data-lucide="eye" size="14"></i></button>
                            <button class="action-btn action-btn-shift" title="Change Shift" onclick="openQuickShiftModal('${emp.id}', '${emp.shift_id || ''}', '${empNameEscaped}')"><i data-lucide="clock" size="14"></i></button>
                            <button class="action-btn action-btn-salary" title="Salary Increment" onclick="openQuickSalaryModal('${emp.id}', '${emp.salary || 0}', '${empNameEscaped}')"><i data-lucide="trending-up" size="14"></i></button>
                            <button class="action-btn action-btn-edit" title="Full Edit Profile" onclick="openEditEmployeeModal('${emp.id}')"><i data-lucide="user-pen" size="14"></i></button>
                    ...
                    <td>
                        <a href="${profileUrl}" class="emp-profile no-decoration hover-opacity">
                            ...
                            <div class="emp-info">
                                ...
                                <span class="email">EMP-${String(emp.id).padStart(3, '0')}</span>
                            </div>
                        </a>
                    </td>
    ...
    window.openAddEmployeeModal = () => {
        ...
        document.getElementById('email_verify_msg').textContent = '';
        const addEmailField = document.getElementById('add_email');
        if (addEmailField) addEmailField.style.borderColor = '';
        // Reset file status
        ...
        try {
            ...
            if (result.status === 'success') {
                ...
                if (document.getElementById('edit_role')) {
                    document.getElementById('edit_role').value = data.role || 'Employee';
                }
                ...
                const accTypeElem = document.getElementById('edit_account_type');
                if (accTypeElem) {
                    let rawAccType = (data.account_type || 'IBN').trim();
                    accTypeElem.value = rawAccType;
                    if (!accTypeElem.value) {
                        for (let opt of accTypeElem.options) {
                            if (opt.value && rawAccType.toLowerCase().includes(opt.value.toLowerCase())) {
                                accTypeElem.value = opt.value;
                                break;
                            }
                        }
                    }
                ...
                const updateFileStatus = (type, path, isOther = false) => {
                    ...
                    if (path && ((!isOther) || (isOther && JSON.parse(path || '[]').length > 0))) {
                        ...
                        filenameLabel.textContent = type === 'resume' ? "Choose Resume" : (type === 'id' ? "Upload ID Card" : "Choose Files");
                        if (infoLabel) {
...
// --- Quick Shift & Salary Increment Modal Handlers ---
window.openQuickShiftModal = async (empId, currentShiftId, empName) => {
    const modal = document.getElementById('quickShiftModal');
    if (!modal) return;

    document.getElementById('qs_employee_id').value = empId;
    document.getElementById('qs_employee_name').value = empName || 'Employee #' + empId;

    const select = document.getElementById('qs_shift_id');
    if (select) {
        select.innerHTML = '<option value="">Loading shifts...</option>';
        try {
            const res = await fetch('/assets/api/employee_handler.php?action=fetch_requirements');
            const data = await res.json();
            if (data.status === 'success' && data.shifts) {
                const formatTime = (t) => {
                    if (!t) return '';
                    let [h, m] = t.split(':');
                    h = parseInt(h);
                ...
                select.innerHTML = '<option value="">Select Shift</option>' +
                    data.shifts.map(s => {
                        const timing = (s.start_time && s.end_time) ? ` (${formatTime(s.start_time)} - ${formatTime(s.end_time)})` : '';
                        const sel = String(s.id) === String(currentShiftId) ? 'selected' : '';
                        return `<option value="${s.id}" ${sel}>${s.name}${timing}</option>`;
                    }).join('');
            }
        } catch (e) {
            console.error('Error fetching shifts for quick modal:', e);
        }
    }
...
window.closeQuickShiftModal = () => {
    const modal = document.getElementById('quickShiftModal');
    if (modal) modal.classList.remove('active');
};

window.openQuickSalaryModal = (empId, currentSalary, empName) => {
    const modal = document.getElementById('quickSalaryModal');
    if (!modal) return;

    const currSal = parseFloat(currentSalary) || 0;
    document.getElementById('qi_employee_id').value = empId;
    document.getElementById('qi_employee_name').value = empName || 'Employee #' + empId;
    document.getElementById('qi_current_salary_display').value = currSal.toLocaleString('en-US');
    document.getElementById('qi_increment_amount').value = '';
    document.getElementById('qi_new_salary').value = '';
    document.getElementById('qi_percentage_label').textContent = '+0%';
    document.getElementById('qi_effective_date').value = new Date().toISOString().split('T')[0];

    modal.classList.add('active');
    if (typeof lucide !== 'undefined') lucide.createIcons();
};

window.closeQuickSalaryModal = () => {
    const modal = document.getElementById('quickSalaryModal');
    if (modal) modal.classList.remove('active');
};

// Form event listeners for quick modals
document.addEventListener('DOMContentLoaded', () => {
    // Quick Shift Form Submit
    const quickShiftForm = document.getElementById('quickShiftForm');
    if (quickShiftForm) {
        quickShiftForm.addEventListener('submit', async (e) => {
            e.preventDefault();
            const btn = quickShiftForm.querySelector('button[type="submit"]');
            if (btn) btn.disabled = true;

            try {
                const formData = new FormData(quickShiftForm);
                const res = await fetch('/assets/api/employee_handler.php?action=quick_update_shift', {
                    method: 'POST',
                    body: formData
                });
                const data = await res.json();
                if (data.status === 'success') {
                    ...
                    if (typeof showToast === 'function') showToast(data.message || 'Error updating shift', 'error');
                    else alert(data.message);
                }
            } catch (err) {
                if (typeof showToast === 'function') showToast('Server error while updating shift.', 'error');
            } finally {
    ...
    // Quick Salary Form Logic & Calculation
    const incInput = document.getElementById('qi_increment_amount');
    const newSalInput = document.getElementById('qi_new_salary');
    const pctLabel = document.getElementById('qi_percentage_label');

    function calculateSalaryIncrement() {
        const currSal = parseFloat((document.getElementById('qi_current_salary_display').value || '0').replace(/,/g, '')) || 0;
        let incAmt = parseFloat(incInput ? incInput.value : 0) || 0;
        
        if (incAmt > 0) {
            const newSal = currSal + incAmt;
            if (newSalInput) newSalInput.value = newSal;
            const pct = currSal > 0 ? ((incAmt / currSal) * 100).toFixed(1) : 0;
            if (pctLabel) pctLabel.textContent = `+${pct}% (+PKR ${incAmt.toLocaleString('en-US')})`;
        } else {
            if (newSalInput) newSalInput.value = '';
            if (pctLabel) pctLabel.textContent = '+0%';
        }
    }

    if (incInput) incInput.addEventListener('input', calculateSalaryIncrement);

    if (newSalInput) {
        newSalInput.addEventListener('input', () => {
            const currSal = parseFloat((document.getElementById('qi_current_salary_display').value || '0').replace(/,/g, '')) || 0;
            const newSal = parseFloat(newSalInput.value) || 0;
            if (newSal > currSal) {
                const incAmt = newSal - currSal;
                if (incInput) incInput.value = incAmt;
                const pct = currSal > 0 ? ((incAmt / currSal) * 100).toFixed(1) : 0;
                if (pctLabel) pctLabel.textContent = `+${pct}% (+PKR ${incAmt.toLocaleString('en-US')})`;
            } else {
                if (pctLabel) pctLabel.textContent = '+0%';
    ...
    const quickSalaryForm = document.getElementById('quickSalaryForm');
    if (quickSalaryForm) {
        quickSalaryForm.addEventListener('submit', async (e) => {
            e.preventDefault();
            const btn = quickSalaryForm.querySelector('button[type="submit"]');
            if (btn) btn.disabled = true;

            try {
                const formData = new FormData(quickSalaryForm);
                const res = await fetch('/assets/api/employee_handler.php?action=quick_salary_increment', {
                    method: 'POST',
                    body: formData
                });
                const data = await res.json();
                if (data.status === 'success') {
                    ...
                    if (typeof showToast === 'function') showToast(data.message || 'Error applying salary increment', 'error');
                    else alert(data.message);
                }
            } catch (err) {
                if (typeof showToast === 'function') showToast('Server error while applying salary increment.', 'error');
            } finally {
                if (btn) btn.disabled = false;
            }
        });
...
// --- Inline Profile Edit Mode Handlers ---
window.toggleProfileInlineEdit = (enable) => {
    const viewVals = document.querySelectorAll('.profile-view-val');
    const editInputs = document.querySelectorAll('.profile-edit-input');
    const toggleBtn = document.getElementById('toggleInlineEditBtn');
    const actionBtns = document.getElementById('inlineEditActionBtns');

    if (enable) {
        viewVals.forEach(el => el.classList.add('hidden'));
        editInputs.forEach(el => el.classList.remove('hidden'));
        if (toggleBtn) toggleBtn.classList.add('hidden');
        if (actionBtns) actionBtns.classList.remove('hidden');
        document.querySelectorAll('.profile-main .premium-card, .profile-aside .premium-card').forEach(card => card.classList.add('editing-active'));
    } else {
        viewVals.forEach(el => el.classList.remove('hidden'));
        editInputs.forEach(el => el.classList.add('hidden'));
        if (toggleBtn) toggleBtn.classList.remove('hidden');
        if (actionBtns) actionBtns.classList.add('hidden');
        document.querySelectorAll('.profile-main .premium-card, .profile-aside .premium-card').forEach(card => card.classList.remove('editing-active'));
    }
};

window.saveProfileInlineEdit = async () => {
    const form = document.getElementById('inlineProfileForm');
    if (!form) return;

    const saveBtn = document.getElementById('saveInlineProfileBtn');
    if (saveBtn) saveBtn.disabled = true;

    try {
        const formData = new FormData(form);
        const res = await fetch('/assets/api/employee_handler.php?action=quick_update_profile', {
            method: 'POST',
            body: formData
        });
        const data = await res.json();
        ...
        if (typeof showToast === 'function') showToast('Server error while saving profile.', 'error');
    } finally {
