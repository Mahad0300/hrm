...
document.addEventListener('DOMContentLoaded', function () {
    ...
    async function loadJobs() {
        try {
            const response = await fetch(`${API_HANDLER}?action=fetch_jobs&_=${Date.now()}`);
            if (!response.ok) {
    ...
    let candidates = [];
    let candidateDepartments = [];
    let applyCandidateFilters = null;

    async function loadCandidateDepartments() {
        try {
            ...
            if (result.status === 'success' && Array.isArray(result.departments)) {
                candidateDepartments = result.departments;
                populateCandidateDeptFilter();
            }
        } catch (e) {
        ...
        try {
            ...
            if (result.status === 'success') {
                candidates = result.data.map(c => ({
                    ...
                    name: c.name || '',
                    email: c.email || '',
                    jobTitle: c.job_title || c.title || 'Unknown',
                ...
                populateCandidateDeptFilter();
                if (typeof applyCandidateFilters === 'function') {
    ...
    function candidateMatchesSearch(cand, query) {
        if (!query) return true;
        const q = query.trim().toLowerCase();
        const haystack = [cand.name, cand.jobTitle]
            .map(v => String(v || '').toLowerCase())
            .join(' ');
        return haystack.includes(q);
    }

    function populateCandidateDeptFilter() {
        const filterDept = document.getElementById('filterDept');
        if (!filterDept) return;

        const selected = filterDept.value;
        const options = candidateDepartments.length
            ? candidateDepartments
            : [...new Map(
                candidates
                    .filter(c => c.departmentId && c.department && c.department !== '—')
                    .map(c => [String(c.departmentId), { id: c.departmentId, name: c.department }])
            ).values()];
            ...
            try {
                ...
                if (result.status === 'success') {
                    Swal.fire('Success', 'Job posted successfully!', 'success').then(() => {
                        window.location.href = window.HRM && window.HRM.url ? window.HRM.url('jobs') : 'jobs';
                    });
                } else {
    ...
    if (jobGridBody) {
        function renderJobs() {
            ...
            if (filteredJobs.length === 0) {
                jobGridBody.innerHTML = `
                    <div class="flex-column flex-center text-center p-80 text-light" style="grid-column: 1/-1; width: 100%; min-height: 250px; display: flex; flex-direction: column; justify-content: center; align-items: center; gap: 12px;">
                        <i data-lucide="briefcase" style="width: 48px; height: 48px; opacity: 0.5; color: var(--text-light, #94a3b8);"></i>
                        <p class="m-0 font-16 italic" style="color: var(--text-light, #94a3b8);">No jobs found matching your criteria.</p>
                    </div>
                `;
                if (window.lucide) {
                ...
                <article class="job-post-card">
                    ...
                    <footer class="job-post-card__footer">
                        ...
                        <div class="job-post-card__actions">
                            ...
                            <a href="${window.HRM && window.HRM.url ? window.HRM.url('jobs/edit') : 'jobs/edit'}?id=${safeJobId}" class="action-btn" title="Edit Job"><i data-lucide="edit-2"></i></a>
        ...
        window.viewJobDetails = async function (jobId) {
            try {
                ...
                document.getElementById('detailEditBtn').onclick = () => { window.location.href = (window.HRM && window.HRM.url ? window.HRM.url('jobs/edit') : 'jobs/edit') + `?id=${job.id}`; };
    ...
    window.copyJobLink = function (id, title) {
        const baseUrl = window.HRM && window.HRM.url ? window.HRM.url('job-apply') : window.location.origin + '/job-apply';
        ...
        if (navigator.clipboard && window.isSecureContext) {
            navigator.clipboard.writeText(link).then(notify).catch(() => fallbackCopy(link, notify));
        } else {
            fallbackCopy(link, notify);
        }
    };
    ...
    function fallbackCopy(text, cb) {
        ...
        textarea.select();
        let copyOk = false;
        try { copyOk = document.execCommand('copy'); } catch(e) {}
        document.body.removeChild(textarea);
        if (copyOk && cb) {
            cb();
        } else {
            prompt('Copy Job Link:', text);
        }
    }
    ...
    if (candidateTableBody) {
        function renderCandidates(filteredData = candidates) {
            ...
            if (totalRows === 0) {
                candidateTableBody.innerHTML = `<tr><td colspan="6" class="text-center p-40 text-light italic">No candidates found.</td></tr>`;
                return;
            }
            ...
            candidateTableBody.innerHTML = currentCands.map(cand => {
                ...
                <tr>
                    ...
                    <td class="font-14">${escapeHtml(cand.jobTitle)}</td>
                    <td class="font-14">${escapeHtml(cand.department)}</td>
                    ...
                    <td class="text-right px-30">
                        <a href="${window.HRM ? window.HRM.baseUrl.replace(/\/(admin|hr)$/, '') + '/recruitment/detail' : '/recruitment/detail'}?name=${encodeURIComponent(String(cand.name).toLowerCase().replace(/[^a-z0-9]+/gi, '-').replace(/^-+|-+$/g, ''))}" class="action-btn no-bg border-0" title="View details">
        ...
        function applyFilters() {
            ...
            let filtered = candidates.filter(cand => {
                const matchesSearch = candidateMatchesSearch(cand, query);
                const matchesStatus = (status === '' || status === 'All Status') ? true : cand.status === status;
                const matchesDept = !dept || String(cand.departmentId) === String(dept);
        ...
        applyCandidateFilters = applyFilters;
        ...
        const interviewLocGroup = document.getElementById('interviewLocationGroup');
        const interviewLocInput = document.getElementById('interviewLocation');

        if (interviewTypeEl && interviewLocGroup) {
            interviewTypeEl.onchange = function() {
                if (this.value === 'Online') {
                    interviewLocGroup.classList.remove('hidden');
                    if (interviewLocInput) interviewLocInput.required = true;
                } else {
                    interviewLocGroup.classList.add('hidden');
                    if (interviewLocInput) {
                        interviewLocInput.required = false;
                        interviewLocInput.value = '';
                    }
                }
            };
        }
        ...
        if (interviewForm) {
            interviewForm.onsubmit = function (e) {
                ...
                const interviewType = document.getElementById('interviewType')?.value || 'Onsite';
                const location = document.getElementById('interviewLocation')?.value || '';
                const sendEmail = document.getElementById('interviewSendEmail')?.checked ? 1 : 0;
                ...
                fetch(API_HANDLER, { method: 'POST', body: fd })
                    .then(res => res.json())
                    .then(res => {
                        if (res.status === 'success') {
    ...
    if (applyForm) {
            ...
            try {
                ...
                if (result.status === 'success') {
                    ...
                    if (intro) {
                        intro.className = 'ja-closed-notice';
                        intro.innerHTML =
                            '<i data-lucide="ban" size="20"></i><span>The requested job posting could not be found or is no longer active.</span>';
                        if (typeof lucide !== 'undefined') lucide.createIcons();
                    }
                }
            } catch (err) {
        ...
        applyForm.onsubmit = async function (e) {
            ...
            try {
                ...
                if (result.status === 'success') {
                    var next = (window.HRM && window.HRM.url ? window.HRM.url('job-apply') : 'job-apply') + '?submitted=1';
    ...
    window.refreshJobPostings = loadJobs;
    window.refreshCandidates = loadCandidates;
});
