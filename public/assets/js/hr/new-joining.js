...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (candEmail && candEmailFeedback) {
        candEmail.addEventListener('blur', async function() {
            ...
            try {
                let checkUrl = `/assets/api/employee_handler.php?action=check_email&email=${encodeURIComponent(email)}`;
                if (hiddenId) {
                    checkUrl += `&exclude_id=${encodeURIComponent(hiddenId)}`;
                }
                ...
                <div class="announcement-card it-dept new-joining-card">
                    ...
                    <div class="announcement-footer">
                        ...
                        <div class="new-joining-actions">
                            ...
                            <button class="action-btn action-btn-delete" data-hr-perm-action="delete" onclick="rejectPendingCandidate(${emp.id})" type="button" title="Reject candidate">
...
async function openHiringModal(empId) {
    try {
        ...
        if (result.status === 'success') {
            ...
            const candEmailFb = document.getElementById('cand_email_feedback');
            if (candEmailFb) candEmailFb.textContent = '';
            const candEmailInp = document.getElementById('cand_email');
            if (candEmailInp) candEmailInp.style.borderColor = '';
...
async function rejectPendingCandidate(empId) {
    const result = await Swal.fire({
        title: 'Reject Candidate?',
        text: 'Are you sure you want to remove this pending joining application?',
        icon: 'warning',
    ...
    try {
        ...
        if (data.status === 'success') {
            Swal.fire({
                title: 'Removed!',
                text: 'Pending candidate application has been removed.',
                icon: 'success',
                confirmButtonColor: '#6c4cf1',
                timer: 1800,
            ...
            Swal.fire('Error', data.message || 'Failed to remove candidate.', 'error');
        }
    } catch (error) {
