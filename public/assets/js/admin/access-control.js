document.addEventListener('DOMContentLoaded', () => {
    ...
    const selfCheckinToggle = document.getElementById('selfCheckinToggle');
    const selfCheckinStatusText = document.getElementById('selfCheckinStatusText');

    selfCheckinToggle?.addEventListener('change', async function () {
        const isChecked = this.checked;
        if (selfCheckinStatusText) {
            selfCheckinStatusText.textContent = isChecked ? 'Show Button (Enabled)' : 'Hide Button (Disabled)';
            selfCheckinStatusText.style.color = isChecked ? '#16a34a' : '#ef4444';
        }
        try {
            const body = new FormData();
            body.append('action', 'save_permissions');
            body.append('permissions', JSON.stringify(collectPermissions()));
            body.append('enable_self_checkin', isChecked ? '1' : '0');
            const chatroxInput = document.getElementById('chatroxUrlInput');
            if (chatroxInput) {
                body.append('chatrox_url', chatroxInput.value.trim());
            }
            const res = await fetch(API, { method: 'POST', body });
            const json = await res.json();
            if (json.status === 'success') {
                showToast(isChecked ? 'Self Check-In button enabled for everyone.' : 'Self Check-In button hidden for everyone.', 'success');
            } else {
                showToast(json.message || 'Failed to update setting.', 'error');
            }
        } catch (err) {
            showToast('Failed to save Check-In setting.', 'error');
        }
    });
    ...
    async function loadPermissions() {
        try {
            ...
            if (selfCheckinToggle) {
                const isEnabled = json.enable_self_checkin !== '0' && json.enable_self_checkin !== 0;
                selfCheckinToggle.checked = isEnabled;
                if (selfCheckinStatusText) {
                    selfCheckinStatusText.textContent = isEnabled ? 'Show Button (Enabled)' : 'Hide Button (Disabled)';
                    selfCheckinStatusText.style.color = isEnabled ? '#16a34a' : '#ef4444';
                }
            }
            const chatroxInput = document.getElementById('chatroxUrlInput');
            if (chatroxInput && json.chatrox_url !== undefined) {
                chatroxInput.value = json.chatrox_url;
            }
        } catch (err) {
    ...
    saveBtn?.addEventListener('click', async () => {
        ...
        try {
            ...
            body.append('permissions', JSON.stringify(collectPermissions()));
            if (selfCheckinToggle) {
                body.append('enable_self_checkin', selfCheckinToggle.checked ? '1' : '0');
            }
            const chatroxInput = document.getElementById('chatroxUrlInput');
            if (chatroxInput) {
                body.append('chatrox_url', chatroxInput.value.trim());
