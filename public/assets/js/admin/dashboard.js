...
function fetchAdminDashboard() {
    fetch('/assets/api/stats_handler.php?action=get_admin_dashboard')
        .then((res) => res.json())
        .then((res) => {
...
function bindFeeds(feeds) {
    ...
    renderFeedList(
        ...
        (j) => {
            ...
            return feedItemHtml(baseUrl + '/new-joining', name, meta, 'dash-feed-dot--primary');
        },
    ...
    renderFeedList(
        ...
        feeds.announcements,
        (a) => feedItemHtml(baseUrl + '/announcements', a.title, 'Posted ' + timeAgoClient(a.created_at), null, false),
    ...
    renderFeedList(
        ...
        (n) => {
            ...
            return feedItemHtml(baseUrl + '/notifications', n.title, msg + (n.message && n.message.length > 50 ? '…' : '') + ' · ' + timeAgoClient(n.created_at), null, false);
        },
...
function bindKpiList(goals) {
    ...
    if (!el) return;
    if (!goals.length) {
        el.innerHTML = '<p class="font-13 text-light py-20 mb-0">No KPI goals recorded yet. <a href="kpi-management">Add reviews</a></p>';
        return;
    }
