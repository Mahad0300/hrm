...
// Logical Date Logic: If before 9 AM, count today as yesterday
$today = date('H') < 9 ? date('Y-m-d', strtotime('-1 day')) : date('Y-m-d');

try {
    switch ($action) {
        case 'get_overview':
            // 1. Total Active Employees (Employee role only ΓÇö excludes Admin & HR)
        ...
        case 'get_personal_overview':
            ...
            // 4. Today's or Active Attendance Details for Logical Today
            $stmt = $pdo->prepare("
