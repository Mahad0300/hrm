...
try {
    if ($action === 'fetch') {
        ...
        if ($search) {
            $searchParam = '%' . $search . '%';
            $where .= " AND (e.first_name LIKE ? OR e.middle_name LIKE ? OR e.last_name LIKE ? OR CAST(e.id AS CHAR) LIKE ? OR CONCAT('EMP-', LPAD(e.id, 3, '0')) LIKE ? OR CONCAT('EMP-0', e.id) LIKE ?)";
            $params[] = $searchParam;
            $params[] = $searchParam;
            $params[] = $searchParam;
            $params[] = $searchParam;
