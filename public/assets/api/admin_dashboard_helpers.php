...
/**
 * Admin dashboard aggregates ΓÇö read-only queries, no side effects.
 */

if (!function_exists('adminDashboardLogicalDate')) {
    function adminDashboardLogicalDate() {
        return date('H') < 9 ? date('Y-m-d', strtotime('-1 day')) : date('Y-m-d');
    }
}
...
if (!function_exists('buildPayrollWeeklyAttendanceTrend')) {
    /**
     * Current calendar week MonΓÇôFri only; days outside payroll range or in the future are null.
     */
...
if (!function_exists('buildAdminDashboardPayload')) {
    function buildAdminDashboardPayload(PDO $pdo, $user_id) {
        ...
        // Total employees ΓÇö Employee role only (excludes Admin, HR, Pending, Exit)
        ...
        $kpi_on_track = 0;
        try {
            $stmt = $pdo->query("SELECT ROUND(AVG(overall_score)) FROM kpi_reviews");
            $kpi_avg = $stmt ? $stmt->fetchColumn() : null;
        ...
        // Turnover ΓÇö exits last 6 months
        $turnover_labels = [];
        $turnover_values = [];
        for ($m = 5; $m >= 0; $m--) {
        ...
        // KPI snapshot (latest items)
        $kpi_rows = [];
        try {
            $stmt = $pdo->query("
                SELECT kri.item_name AS goal_name, kri.target_weight AS target_score, kri.achieved_score
                FROM kpi_review_items kri
                INNER JOIN kpi_reviews kr ON kr.id = kri.review_id
                ORDER BY kr.created_at DESC, kri.id ASC
                LIMIT 4
            ");
            if ($stmt) {
