...
try {
    switch ($action) {
        case 'fetch':
            ...
            $is_privileged = in_array($user_role, ['Admin', 'HR'], true);
            ...
            if ($is_privileged) {
                ...
                $stmt = $pdo->prepare("SELECT e.*, CONCAT(emp.first_name, ' ', emp.last_name) as author_name 
                                    FROM events e 
                                    LEFT JOIN employees emp ON e.created_by = emp.id 
                                    WHERE LOWER(e.target_dept) IN ('everyone', 'all') 
                                       OR FIND_IN_SET(?, REPLACE(e.target_dept, ', ', ',')) > 0
                                    ORDER BY e.event_date DESC");
        ...
        case 'save':
            ...
            \App\Helpers\WebSocketHelper::broadcast('calendar_updated');
