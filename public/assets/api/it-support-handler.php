...
if (!$is_it_staff && (stripos($dept_name, 'IT') !== false || stripos($dept_name, 'Information Technology') !== false)) {
    $is_it_staff = true;
}
...
try {
    ...
    switch ($action) {
        ...
        case 'create_ticket':
            ...
            // Fetch employee's name who created the ticket
            $creator_stmt = $pdo->prepare("SELECT first_name, last_name FROM employees WHERE id = ?");
            $creator_stmt->execute([$user_id]);
            $creator_row = $creator_stmt->fetch();
            $creator_name = trim(($creator_row['first_name'] ?? 'Employee') . ' ' . ($creator_row['last_name'] ?? ''));

            // [ACTIVITY LOG]
            logActivity($user_id, "Created IT Ticket", "IT Support", "Created support ticket #$ticket_id: $subject");

            // [NOTIFICATIONS] Fetch IT department IDs to notify Admin, HR, and IT staff
            $it_depts_stmt = $pdo->query("SELECT id FROM departments WHERE name LIKE '%IT%' OR name LIKE '%Information Technology%'");
            $it_dept_ids = $it_depts_stmt->fetchAll(PDO::FETCH_COLUMN) ?: [];
            
            if (!empty($it_dept_ids)) {
                $recipients_query = "SELECT id FROM employees WHERE (role IN ('Admin', 'HR') OR department_id IN (" . implode(',', array_map('intval', $it_dept_ids)) . ")) AND status = 'Active' AND deleted_at IS NULL";
            } else {
                $recipients_query = "SELECT id FROM employees WHERE role IN ('Admin', 'HR') AND status = 'Active' AND deleted_at IS NULL";
            }
            
            $recipients_stmt = $pdo->query($recipients_query);
            $recipient_ids = $recipients_stmt->fetchAll(PDO::FETCH_COLUMN) ?: [];
            
            // Exclude the creator from the notification recipients
            $recipient_ids = array_filter($recipient_ids, function($id) use ($user_id) {
                return (int)$id !== (int)$user_id;
            });
            
            if (!empty($recipient_ids)) {
                addNotification(
                    array_values($recipient_ids),
                    "New IT Support Ticket",
                    "A new IT ticket (#$ticket_id) has been created by $creator_name.",
                    "it-support",
                    "System",
                    $user_id
                );
            ...
            \App\Helpers\WebSocketHelper::broadcast('support_updated', ['ticket_id' => $ticket_id]);
        ...
        case 'send_message':
            ...
            // WebSocket push to update active support boards
            $recipientId = ($user_id === $ticket_creator) ? (int)$ticketRow['assigned_to'] : $ticket_creator;
            if ($recipientId > 0) {
                \App\Helpers\WebSocketHelper::sendToUser($recipientId, 'support_updated', ['ticket_id' => $ticket_id]);
            } else {
                \App\Helpers\WebSocketHelper::broadcast('support_updated', ['ticket_id' => $ticket_id]);
            }
        ...
        case 'get_it_staff':
            ...
            $stmt = $pdo->prepare("SELECT e.id, TRIM(CONCAT_WS(' ', e.first_name, e.middle_name, e.last_name)) as name FROM employees e LEFT JOIN departments d ON e.department_id = d.id WHERE d.name LIKE '%IT%' OR d.name LIKE '%Information Technology%'");
            $stmt->execute();
