...
function respondAndContinue(array $data) {
...
// Public actions that don't require authentication (used by job-apply.php & walk-in.php)
$public_actions = ['submit_application', 'submit_walk_in', 'fetch_jobs', 'fetch_job_detail', 'fetch_interviews'];
    ...
    switch ($action) {
        ...
        case 'fetch_jobs':
            $stmt = $pdo->query("
                ...
                (SELECT COUNT(DISTINCT c.id) FROM candidates c WHERE c.job_id = j.id AND c.deleted_at IS NULL AND (c.status = 'Interview' OR c.id IN (SELECT candidate_id FROM interviews))) as interview_count
                FROM jobs j
        ...
        case 'fetch_candidate_detail':
            ...
            if ($candidate) {
                ...
                $iStmt = $pdo->prepare("SELECT id, DATE(interview_date) as date, TIME_FORMAT(interview_date, '%H:%i') as time, interview_type, location, feedback FROM interviews WHERE candidate_id = ? ORDER BY created_at DESC LIMIT 1");
        ...
        case 'fetch_candidates':
            $stmt = $pdo->query("
                SELECT c.*, j.title as job_title, j.department_id, d.name as department_name
                FROM candidates c
                LEFT JOIN jobs j ON c.job_id = j.id
                LEFT JOIN departments d ON j.department_id = d.id
                WHERE c.deleted_at IS NULL AND (c.is_walk_in IS NULL OR c.is_walk_in = 0)
        ...
        case 'fetch_walk_in_candidates':
            $stmt = $pdo->query("
                SELECT c.*, j.title as job_title, j.department_id, d.name as department_name
                FROM candidates c
                LEFT JOIN jobs j ON c.job_id = j.id
                LEFT JOIN departments d ON j.department_id = d.id
                WHERE c.deleted_at IS NULL AND c.is_walk_in = 1
                ORDER BY c.created_at DESC
            ");
            $candidates = $stmt->fetchAll();
            echo json_encode(['status' => 'success', 'data' => $candidates]);
        ...
        case 'submit_walk_in':
            ...
            if (!$name || !$email || !$phone || !$cnic || !$address || !$job_id) {
            ...
            try {
                ...
                // Duplicate Check (Email, Phone, CNIC, Address)
                $duplicate_of = null;
                ...
                if ($orig) {
                    ...
                    if ($address && $address === $orig['address']) $matched_fields[] = 'Address';
                ...
                if (isset($_FILES['resume']) && $_FILES['resume']['error'] === UPLOAD_ERR_OK) {
                    ...
                    \App\Helpers\StorageHelper::ensureDirectory('uploads/candidates/resumes/');
                    $fileName = 'WALK_' . uniqid() . '.' . $ext;
                    $targetPath = \App\Helpers\StorageHelper::diskPath('uploads/candidates/resumes/' . $fileName);
                    if (move_uploaded_file($_FILES['resume']['tmp_name'], $targetPath)) {
                        $resumePath = 'uploads/candidates/resumes/' . $fileName;
                    }
                }

                $stmt = $pdo->prepare("
                    INSERT INTO candidates (name, email, phone, cnic_number, address, job_id, applied_date, status, duplicate_of, duplicate_reason, resume_path, is_walk_in, created_at)
                    VALUES (?, ?, ?, ?, ?, ?, CURDATE(), ?, ?, ?, ?, 1, NOW())
                ");
                $stmt->execute([$name, $email, $phone, $cnic, $address, $job_id, $status, $duplicate_of, $duplicate_reason, $resumePath]);
                $candId = $pdo->lastInsertId();

                // Insert initial history
                $historyMsg = $status === 'Duplicated' ? "Walk-In Registration - Duplicated: $duplicate_reason" : 'Registered via Walk-In Interview Form';
                $hStmt = $pdo->prepare("INSERT INTO candidate_history (candidate_id, status_from, status_to, feedback, created_by, created_at) VALUES (?, 'New', ?, ?, 0, NOW())");
                $hStmt->execute([$candId, $status, $historyMsg]);
                ...
                respondAndContinue(['status' => 'success', 'message' => 'Walk-in application registered successfully.', 'id' => $candId, 'is_duplicate' => ($status === 'Duplicated')]);
            } catch (Exception $e) {
        ...
        case 'delete_candidate':
            ...
            if (!$cand_id) throw new Exception("Candidate ID is required.");

            $stmt = $pdo->prepare("UPDATE candidates SET deleted_at = NOW() WHERE id = ?");
            $stmt->execute([$cand_id]);
        ...
        case 'submit_application':
            ...
            try {
                ...
                if (isset($_FILES['resume']) && $_FILES['resume']['error'] === UPLOAD_ERR_OK) {
                    ...
                    \App\Helpers\StorageHelper::ensureDirectory('uploads/candidates/resumes/');
                    $target_file = \App\Helpers\StorageHelper::diskPath('uploads/candidates/resumes/' . $file_name);
                ...
                // Broadcast WebSockets to update Job Postings APPS counts and Candidates list instantly
                \App\Helpers\WebSocketHelper::broadcast('jobs_updated');
                \App\Helpers\WebSocketHelper::broadcast('candidates_updated');
        ...
        case 'update_candidate_status':
            ...
            // If status is Shortlisted and interview date & time are provided, insert record into interviews table
            if ($status === 'Shortlisted' && !empty($_POST['date']) && !empty($_POST['time'])) {
                $interview_datetime = $_POST['date'] . ' ' . $_POST['time'];
                $interview_type = $_POST['interview_type'] ?? 'Onsite';
                if (!in_array($interview_type, ['Online', 'Onsite'], true)) {
                    $interview_type = 'Onsite';
                }
                $location = trim($_POST['location'] ?? '');

                $iStmt = $pdo->prepare("INSERT INTO interviews (candidate_id, interview_date, interview_type, location, feedback, status, created_at) VALUES (?, ?, ?, ?, ?, 'Scheduled', NOW())");
                $iStmt->execute([$id, $interview_datetime, $interview_type, $location, $feedback]);
            }
            ...
            // Trigger WebSocket updates immediately
            \App\Helpers\WebSocketHelper::broadcast('candidates_updated');
            \App\Helpers\WebSocketHelper::broadcast('jobs_updated');
            \App\Helpers\WebSocketHelper::broadcast('interviews_updated');

            respondAndContinue(['status' => 'success', 'message' => 'Status updated successfully']);
            
            $extraData = [];
            if (isset($_POST['date'])) $extraData['date'] = $_POST['date'];
            if (isset($_POST['time'])) $extraData['time'] = $_POST['time'];
            if (isset($_POST['interview_type'])) $extraData['interview_type'] = $_POST['interview_type'];
            if (isset($_POST['shift_id']) && !empty($_POST['shift_id'])) {
                $extraData['shift_id'] = $_POST['shift_id'];
                $sStmt = $pdo->prepare("SELECT start_time FROM shifts WHERE id = ?");
                $sStmt->execute([$_POST['shift_id']]);
                $shiftStartTime = $sStmt->fetchColumn();
                if ($shiftStartTime) {
                    $extraData['reporting_time'] = date('g:i A', strtotime($shiftStartTime));
                }
            }

            if ($send_email === 1) {
                sendCandidateStatusEmail($id, $status, $extraData);
            }
            break;
        ...
        case 'schedule_interview':
            ...
            $stmt = $pdo->prepare("INSERT INTO interviews (candidate_id, interview_date, interview_type, location, feedback, status, created_at) VALUES (?, ?, ?, ?, ?, 'Scheduled', NOW())");
            $stmt->execute([$candidate_id, $interview_datetime, $interview_type, $location, $feedback]);
            ...
            $locMeta = $location ? " ($interview_type: $location)" : " ($interview_type)";
            ...
            $hStmt->execute([$candidate_id, $prevStatus, "Interview scheduled for $formattedTime$locMeta. $feedback", $user_id]);

            // [LOG]
            $c_stmt = $pdo->prepare("SELECT name FROM candidates WHERE id = ?");
            $c_stmt->execute([$candidate_id]);
            $c_name = $c_stmt->fetchColumn();
            logActivity($user_id, "Scheduled Interview", "Job Management", "Scheduled an $interview_type interview session for candidate '$c_name' on " . date('M d, Y', strtotime($date)) . " at $time.");
            ...
            respondAndContinue(['status' => 'success', 'message' => 'Interview scheduled successfully']);

            if ($send_email === 1) {
                sendCandidateStatusEmail($candidate_id, 'Interview', [
                    'date' => $date, 
                    'time' => $time,
        ...
        case 'reschedule_interview':
            ...
            // Update Interview
            $interview_datetime = $date . ' ' . $time;
            $stmt = $pdo->prepare("UPDATE interviews SET interview_date = ?, interview_type = ?, location = ?, feedback = ?, updated_at = NOW() WHERE id = ?");
            $stmt->execute([$interview_datetime, $interview_type, $location, $feedback, $interview_id]);
            ...
            if ($target_candidate_id) {
                ...
                $history_msg = "Interview rescheduled$locMeta. Previous: $prevFormatted. New: $newFormatted. Notes: $feedback";
                ...
                $c_name = $c_stmt->fetchColumn();
                logActivity($user_id, "Rescheduled Interview", "Job Management", "Rescheduled the $interview_type interview session for '$c_name' to " . date('M d, Y', strtotime($date)) . " at $time.");
            }
            ...
            respondAndContinue(['status' => 'success', 'message' => 'Interview rescheduled successfully']);

            if ($target_candidate_id && $send_email === 1) {
                sendCandidateStatusEmail($target_candidate_id, 'Interview', [
                    'date' => $date, 
                    'time' => $time,
                    'interview_type' => $interview_type,
                    'location' => $location
